...
class TaskManager:
    ...
    def _distribute_command(self, command_sequence, index=None):
        ...
        """
        # Assign a visit_id to this command
            ...
            while True:
                for browser in self.browsers:
                    if browser.ready():
                        ...
                        thread = self._start_thread(browser,
                                                    visit_id,
                                                    command_sequence)
    ...
    def _start_thread(self, browser, visit_id,
                      command_sequence, condition=None):
        ...
        self.sock.send(("INSERT INTO site_visits "
                        "(visit_id, crawl_id, email, site_url) "
                        "VALUES (?,?,?,?)",
                        (visit_id, browser.crawl_id,
                         command_sequence.email, command_sequence.url)))
